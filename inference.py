...
from models.DINet import DINet,DINet1,DINet2,DINet_best,DINet_origin
...
if __name__ == "__main__":
    ...
    for ref_index in ref_index_list:
        ...
        ref_frame_mouth_data=ref_img_crop.copy()
        ref_frame_mouth_data[
        ...
        # ref_frame_mouth_data=np.zeros_like(ref_img_crop)
        ...
        #     :,
        #     ]
        ref_img_mouth_list.append(ref_frame_mouth_data)
    # ref_img_mouth = np.concatenate(ref_img_mouth_list,2)
    ref_clip_data_without_concat=ref_img_list.copy()
    ref_clip_data_without_concat=np.transpose(ref_clip_data_without_concat, (3, 0, 1, 2)) # [3,5,416,320]
    ref_video_frame = np.concatenate(ref_img_list, 2)
    ref_img_tensor = (
        torch.from_numpy(ref_video_frame).permute(2, 0, 1).unsqueeze(0).float().cuda()
    )
    ref_clip_data_without_concat_tensor = (
        torch.from_numpy(ref_clip_data_without_concat).unsqueeze(0).float().cuda()
    )
    ref_img_mouth_tensor = (
        torch.from_numpy(np.reshape(np.array(ref_img_mouth_list),(3,5,416,320))).unsqueeze(0).float().cuda()
    )
    # load pretrained model weight
    logging.info("loading pretrained model from: %s", opt.pretrained_clip_DINet_path)
    model = DINet1(opt.source_channel, opt.ref_channel, opt.audio_channel).cuda()
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        mask = np.ones_like(pre_frame_resize, np.uint8)*255

        # 计算源图像在目标图像上的中心位置
        center_x = (frame_landmark[33, 0] - crop_radius - crop_radius_1_4 + frame_landmark[33, 0] + crop_radius + crop_radius_1_4) // 2
        center_y = (frame_landmark[29, 1] - crop_radius + frame_landmark[29, 1] + crop_radius * 2) // 2
        center = (center_x, center_y)        #     + crop_radius * 2,
        #     frame_landmark[33, 0]
        #     - crop_radius
        #     - crop_radius_1_4, frame_landmark[29, 1])

        # 使用 seamlessClone 函数进行无缝克隆
        # frame_data = cv2.seamlessClone(pre_frame_resize, frame_data, mask, center, cv2.NORMAL_CLONE)
        frame_data[
            frame_landmark[29, 1]
            - crop_radius : frame_landmark[29, 1]
            + crop_radius * 2,
