...
class DINetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        deep_speech_list = []
        reference_clip_list = []
        source_clip_mask1_list = []
        reference_frame_mouth_data_list=[]
        reference_clip_data_without_concat_list=[]
        for source_frame_index in range(2, 2 + 5):
            ...
            source_clip_mask1_list.append(source_image_mask1)
            ...
            reference_frame_mouth_data_each_list = []
            ...
            for reference_anchor in reference_anchor_list:
                ...
                reference_frame_mouth_data_each_list.append(reference_frame_mouth_data)
            reference_frame_mouth_data_list.append(reference_frame_mouth_data_each_list.copy())
            reference_clip_data_without_concat_list.append(reference_frame_list.copy())
        ...
        source_clip_mask1 = np.stack(source_clip_mask1_list, 0)
        ...
        source_clip_mask1 = torch.from_numpy(source_clip_mask1).float().permute(0, 3, 1, 2)
        reference_clip_data_mouth = torch.from_numpy(np.array(reference_frame_mouth_data_list)).permute(0,4,1,2,3)
        # reference_clip_data_mouth = np.transpose(reference_frame_mouth_data_list, (3, 0, 1, 2))
        # reference_clip_data_without_concat=torch.tensor(reference_clip_data_without_concat_list)
        # reference_clip_data_without_concat=np.transpose(reference_clip_data_without_concat, (1, 0, 4, 2,3))
        # reference_clip_data_without_concat=reference_clip_data_without_concat.reshape(3,-1,416,320)
        reference_clip_data_without_concat = torch.from_numpy(np.array(reference_clip_data_without_concat_list)).permute(0,4,1,2,3)
        ...
        # deep_speech_clip=deep_speech_clip.reshape(29,-1)
        # deep_speech_clip = torch.mean(deep_speech_clip,dim=0)
