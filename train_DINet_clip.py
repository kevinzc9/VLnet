...
if __name__ == "__main__":
    ...
    for epoch in range(opt.start_epoch, opt.non_decay + opt.decay + 1):
        ...
        for iteration, data in enumerate(training_data_loader):
            ...
            #[15,3,416,320]
            # source_clip_mask = (
            #     torch.split(source_clip_mask, 1, dim=1)
            #     .float()
            #     .cuda()
            # )
            reference_clip = (
            ...
            # reference_clip_data_mouth = (
            #     torch.cat(torch.split(reference_clip_data_mouth, 1, dim=1), 0)
            #     .squeeze(1)
            #     .float()
            #     .cuda()
            # )
            ...
            # reference_clip_data_without_concat = (
            #     torch.cat(torch.split(reference_clip_data_without_concat, 1, dim=1), 0)
            #     .squeeze(1)
            #     .float()
            #     .cuda()
            # )
            ...
            # reference_clip_data_without_concat = reference_clip_data_without_concat.float().cuda()
            # reference_clip_data_mouth = reference_clip_data_mouth.float().cuda()
            # deep_speech_clip = deep_speech_clip.float().cuda()

            deep_speech_full = deep_speech_full.float().cuda()
            
            
            # deepspeech [15,29,5]
            # reference_clip [3*5,15,104,80]
            # source_image_data [3*5,3,104,80]
            # source_clip_mask [3*5,3,104,80]
            # reference_clip_data_mouth [3*5,3,5,104,80]
            # reference_clip_data_without_concat [3*5,3,5,104,80]
